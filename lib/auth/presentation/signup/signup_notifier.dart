...
class SignupNotifier extends _$SignupNotifier {
  ...
  SignupState build() {
    ...
    _checkNicknameAvailabilityUseCase = ref.watch(checkNicknameAvailabilityUseCaseProvider);
    _checkEmailAvailabilityUseCase = ref.watch(checkEmailAvailabilityUseCaseProvider);
    ...
    _validatePasswordConfirmUseCase = ref.watch(validatePasswordConfirmUseCaseProvider);
    _validateTermsAgreementUseCase = ref.watch(validateTermsAgreementUseCaseProvider);
    ...
    // 폼 입력값 변경 액션 처리
      ...
      case PasswordChanged(:final password):
        state = state.copyWith(
          ...
          passwordConfirmError: state.passwordConfirm.isEmpty ? null :
          await _validatePasswordConfirmUseCase.execute(password, state.passwordConfirm),
        );
  ...
  Future<void> _performNicknameAvailabilityCheck() async {
    ...
    final result = await _checkNicknameAvailabilityUseCase.execute(state.nickname);

    state = state.copyWith(
      nicknameAvailability: result,
      nicknameError: result.hasError
  ...
  Future<void> _performSignup() async {
    // 1. 모든 필드의 유효성 검증
    final nicknameError = await _validateNicknameUseCase.execute(state.nickname);
    final emailError = await _validateEmailUseCase.execute(state.email);
    final passwordError = await _validatePasswordUseCase.execute(state.password);
    final passwordConfirmError = await _validatePasswordConfirmUseCase.execute(
        state.password, state.passwordConfirm);
    final termsError = await _validateTermsAgreementUseCase.execute(state.agreeToTerms);
    ...
    if (nicknameError != null || emailError != null ||
        passwordError != null || passwordConfirmError != null || termsError != null) {
      return;
    }
    ...
    if (state.nicknameAvailability?.value != true || state.emailAvailability?.value != true) {
      return;
    }
